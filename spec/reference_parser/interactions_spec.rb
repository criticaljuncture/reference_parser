...
RSpec.describe "Interactions" do
  SCENERIOS_INTERACTIONS.each do |scenerio|
    it scenerio[:description] do
      result_html = ReferenceParser.new.hyperlink(scenerio[:ex], default: {target: nil, class: nil})

      expect(result_html).to eq(scenerio[:expected_html])
    end
  end
end
