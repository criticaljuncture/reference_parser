...
RSpec.describe ReferenceParser::Url do
  ...
  def drop_zero_width_spaces(text)
    text.gsub("&#8203;", "") # ?
  end
  ...
  xit "handles trailing semicolons" do
  ...
  it "handles EOL" do
    ...
    expect(hyperlink("<p>#{url1}<br />#{url2}<br /></p>")).to eql drop_zero_width_spaces %(<p><a href='#{url1}'>http://api.rubyonrails.com/&#8203;Foo.html</a><br /><a href='#{url2}'>http://www.ruby-doc.org/&#8203;core/&#8203;Bar.html</a><br /></p>)
  end
  ...
  it "handles previously escaped input" do
    ...
    XML
    expect(result).to eql(drop_zero_width_spaces(<<-XML))
      <E T="03">
